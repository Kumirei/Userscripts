...
(function() {
    ...
    function fetch_data() {
        ...
        if (tracker.last_fetch < Date.now() - 1000*60*settings.update_interval) {
            ...
            save();
            let username = $('#current-user a').attr('href').split('/u/')[1];
    ...
    function add_display() {
        ...
        if (!$('#regular_status').length) {
            $('.wanikani-app-nav ul').append('<li id="regular_status" data-highlight="false">Regular Status:<span class="dashboard_bubble">0%</span></li>');
            update_display();
        }
    }
...
Visit Streak:               ${visit_streak}`;
            ...
            let goals = [days_visited/50, likes_given/30, likes_received/20, topics_viewed/topics_goal, posts_read/post_goal].reduce((a, b)=>{return b<a?b:a;});
