...
(function(wkof) {
    window.WKExporter = {export_reviews, export_lessons, export_csv};

    async function export_reviews(name, ids) {
        let reviews = await window.review_cache.get_reviews();
        let wk_items = wkof.ItemData.get_index(await wkof.ItemData.get_items('assignments'), 'subject_id');
    ...
    async function export_lessons(name, ids) {
        let lessons = (await wkof.ItemData.get_items('assignments')).filter(item => item.assignments && item.assignments.started_at && ids.includes(item.id));
