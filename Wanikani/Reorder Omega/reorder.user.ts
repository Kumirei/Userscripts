...
;(async () => {
    ...
    // TODO: Fix typescript types. esp any
    // * and superfluous types
    // * and casting
    // TODO: Clean up omega references
    // TODO: Clean up globals
    // TODO: make use of MS

    // TODO: Descriptions of the settings

    // TODO: Incorporate CSS
    ...
    // Page variables
    let page: 'other' | 'dashboard' | 'reviews' | 'lessons' | 'extra_study',
        currentItemKey: string,
        activeQueueKey: string,
        fullQueueKey: string,
        questionTypeKey: string,
        UIDPrefix: string,
        traceFunctionTest: RegExp
    ...
    if (page === 'extra_study') display_loading()
    ...
    function set_page_variables(): 'other' | 'dashboard' | 'reviews' | 'lessons' | 'extra_study' {
        const path = window.location.pathname
        const extra_study_search = window.location.search === `?title=${encodeURIComponent(script_name)}`
        if (/^\/(DASHBOARD)?$/i.test(path)) page = 'dashboard'
        else if (/REVIEW\/session/i.test(path)) page = 'reviews'
        else if (/LESSONS\/session/i.test(path)) page = 'lessons'
        else if (/EXTRA_STUDY\/session/i.test(path) && extra_study_search) page = 'extra_study'
        else page = 'other'
    ...
    function add_to_extra_study_section(): void {
        const button = $(`<li class="md:mr-3">
            <div class=" border border-blue-300 border-solid rounded flex flex-row ">
                ...
                data-test="extra-study-button">${script_name}
                </a>
            </div>
        </li>`)
    ...
    function install_extra_features(): void {
        ...
        function install_egg_timer(): void {
            if (page !== 'reviews' && page !== 'lessons' && page !== 'extra_study') return
            ...
            const streak = {
                ...
                load: (): void => {
                    const data: StreakTracker = {
                        questions: 0,
                        incorrect: 0,
                        ...JSON.parse(localStorage.getItem(`${script_id}_streak`) ?? '{"streak": 0, "max": 0}'),
                    }
                    streak.current = data
                    streak.prev = data
                },
        ...
        function install_back_to_back(): void {
            ...
            const new_random = function (): number {
                const match = traceFunctionTest.exec(new Error().stack as string)
            ...
            console.log(
                'Beware, "Back To Back" is installed and may cause other scripts using Math.random ' +
                    `in a function called ${traceFunctionTest} to misbehave.`,
            )
            ...
            if (page === 'reviews' || page === 'extra_study') {
                ...
                const callback = () => {
                    $.jStorage.set(currentItemKey, $.jStorage.get<Review.Item[]>(activeQueueKey)[0])
                    $.jStorage.stopListening('activeQueue', callback)
                }
                if ($.jStorage.get<Review.Item[]>(activeQueueKey).length) callback()
        ...
        function install_prioritization(): void {
            ...
            function prioritize(): void {
                ...
                const item = $.jStorage.get<Review.Item>(currentItemKey)
                // Skip if item is a radical, it is already the right question, or no priority is selected
                if (
                    item.type == 'Radical' ||
                    $.jStorage.get<'meaning' | 'reading'>(questionTypeKey) == prio ||
                    'none' == prio
                )
                    return
                const UID = (item.type == 'Kanji' ? 'k' : 'v') + item.id
                const done = $.jStorage.get<Review.AnswersObject>(UIDPrefix + UID)
    ...
    function install_interface(): void {
        page = page as 'reviews' | 'lessons' | 'extra_study'
        ...
        select.val(settings[`active_preset_${page}`]).on('change', (event: any) => {
            page = page as 'reviews' | 'lessons' | 'extra_study'
            // Change in settings then save
            settings[`active_preset_${page}`] = event.currentTarget.value
    ...
    function process_filter(
        action: Settings.FilterOrSortAction,
        items: ItemData.Item[],
    ): KeepAndDiscard<ItemData.Item> {
    ...
    function process_sort_action(action: Settings.FilterOrSortAction, items: ItemData.Item[]): ItemData.Item[] {
    ...
    async function update_queue(items: ItemData.Item[]): Promise<void> {
        const first10 = await get_item_data(items.splice(0, 10))
    ...
    function open_settings() {
        ...
        settings_dialog = new wkof.Settings(config as SettingsModule.Config)
    ...
    function populate_active_preset_options(active_presets: any) {
        for (let [i, preset] of Object.entries(settings.presets) as any) {
    ...
    function refresh_active_preset_selection() {
        for (let type of ['reviews', 'lessons', 'extra_study'])
            populate_preset_pickers(type as 'reviews' | 'lessons' | 'extra_study')

        function populate_preset_pickers(type: 'reviews' | 'lessons' | 'extra_study') {
            const elem: JQuery = $(`#${script_id}_active_preset_${type}`)
            const presets = Object.entries(settings.presets).filter(([i, preset]) => preset.available_on[type])
            const selected = settings[`active_preset_${type}`]
            ...
            if (!selected_available) settings[`active_preset_${type}`] = Number(presets[0][0])
