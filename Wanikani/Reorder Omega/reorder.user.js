...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Page variables
    let page, header_row_location = '.character-header__menu', preset_selection_location = '.character-header__content';
    // Settings objects
    let settings = {};
    let settings_dialog;
    let burn_bell_audio = new Audio(); // Burn bell audio element
    let burn_bell_audio_sources;
    ...
    function load_wkof() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield wkof.ready('Settings,Menu,Jquery').then(load_settings).then(install_menu);
            yield wkof.ready('ItemData,Apiv2');
        });
    }
    yield load_wkof();
    // Install css
    ...
    function init_once() {
        ...
        function install_initializer() {
            ...
            window.addEventListener(`turbo:load`, () => __awaiter(this, void 0, void 0, function* () {
                // timeout of 0 used to delay execution to next tick of event loop to allow turbo events time to settle
                setTimeout(() => __awaiter(this, void 0, void 0, function* () { yield load_wkof(); init(); }), 0);
            }));
        }
        ...
        function install_prioritization() {
            ...
            if (settings.prioritize !== 'none')
                wkQueue.questionOrder = `${settings.prioritize}First`;
        }
            ...
            wkQueue.addPostprocessing((queue) => {
                ...
                for (let item of queue) {
                    ...
                    for (let reading of item.subject.readings || []) {
                        ...
                        if (!sources.length)
                            continue;
                        for (let pronunciation of reading.pronunciations)
        ...
        function install_streak_tracker() {
            ...
            streak = {
                current: {},
                prev: {},
                save: () => localStorage.setItem(`${script_id}_${page}_streak`, JSON.stringify({ streak: streak.current.streak, max: streak.current.max })),
                load: () => {
                    var _a;
                    const data = Object.assign({ questions: 0, incorrect: 0 }, JSON.parse((_a = localStorage.getItem(`${script_id}_${page}_streak`)) !== null && _a !== void 0 ? _a : '{"streak": 0, "max": 0}'));
                    streak.current = data;
                    streak.prev = data;
                },
                undo: () => {
    ...
    function set_page_variables() {
        ...
        if (/^\/(DASHBOARD)?$/i.test(path))
            page = 'dashboard';
        else if (/SUBJECTS\/REVIEW/i.test(path))
            page = 'reviews';
        else if (/SUBJECT-LESSONS/i.test(path))
            page = 'lessons';
        else if (/RECENT-MISTAKES\/.*quiz/i.test(path))
            page = 'extra_study';
        else if (/EXTRA_STUDY/i.test(path))
    ...
    function apply_preset(queue, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const wkQueueItems = new Map();
            for (let item of queue)
                wkQueueItems.set(item.id, item);
            const wkofQueue = yield process_queue(queue.map((item) => item.item));
            ...
            return wkofQueue.map((item) => wkQueueItems.get(item.id) || item.id); // item.id needed for self_study where we convert from WKOF object
        });
    }
    ...
    function process_queue(items) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!preset)
                return [];
            if (page === 'self_study')
                items = yield wkof.ItemData.get_items();
    ...
    function process_preset(preset, items) {
        let result = { keep: items, discard: [], final: [] };
    ...
    function process_filter(action, items) {
        ...
        const filter_func = (item) => xor(action.filter.values.invert, filter.filter_func(filter_value, item));
    ...
    function process_sort_action(action, items) {
        ...
        switch (action.sort.type) {
            ...
            case 'overdue_absolute':
                sort = (a, b) => numerical_sort(calculate_overdue_days(a), calculate_overdue_days(b), action.sort.values.overdue_absolute);
                break;
            case 'critical':
                sort = (a, b) => numerical_sort(+is_critical(a), +is_critical(b), action.sort.values.critical);
                break;
            case 'leech':
                sort = (a, b) => numerical_sort(calculate_leech_score(a), calculate_leech_score(b), action.sort.values.leech);
                break;
            default:
    ...
    function relative_shuffle(arr, distance) {
        const sort_indices = new Map();
        arr.forEach((item, i) => sort_indices.set(item, i + distance * arr.length * Math.random()));
        return arr.sort((a, b) => { var _a, _b; return ((_a = sort_indices.get(a)) !== null && _a !== void 0 ? _a : 0) - ((_b = sort_indices.get(b)) !== null && _b !== void 0 ? _b : 0); });
    }
    ...
    function install_interface() {
        ...
        for (let [i, preset] of Object.entries(settings.presets)) {
            if (preset.available_on[page])
                options.push(`<option value=${i}>${preset.name}</option>`);
        }
        const select = $(`<select id="${script_id}_preset_picker">${options.join('')}</select>`);
        select.val(settings.active_presets[page]).on('change', (event) => {
            page = page;
        ...
        $(body)
            ...
            .append($(`<div id="active_preset" ${!settings.display_selection ? 'class="hidden"' : ''}>Preset: </div>`).append(select));
        if (page === 'lessons') {
            ...
            batch_input.on('change', () => {
                ...
                debounceTimer = setTimeout(() => {
                    page = page;
                    settings.batch_size = wkQueue.lessonBatchSize = $(`#${script_id}_batch_size_input`).val();
                    wkof.Settings.save(script_id);
                    wkQueue.refresh();
                }, 500);
            });
        ...
        function install_streak() {
            ...
            const elem = $(`
                <div id="streak" class="quiz-statistics__item"><div class="quiz-statistics__item-count">
                    ...
                    <div class="count quiz-statistics__item-count-text">${((_a = streak === null || streak === void 0 ? void 0 : streak.current) === null || _a === void 0 ? void 0 : _a.streak) || 0} (${((_b = streak === null || streak === void 0 ? void 0 : streak.current) === null || _b === void 0 ? void 0 : _b.max) || 0})</div>
                </div></div>
                `);
    ...
    function install_filters() {
        ...
        wkof.ItemData.registry.sources.wk_items.filters[`${script_id}_random_interval_spread`] = {
            type: 'number',
            default: 0,
    ...
    function confirm_wkof() {
        if (!wkof) {
            ...
            if (!wkof.version || wkof.version.compare_to(wkof_version_needed) === 'older') {
                let response = confirm(`${script_name} requires WaniKani Open Framework version ${wkof_version_needed} or higher.\nClick "OK" to be forwarded to the update page.`);
                if (response) {
    ...
    function load_settings() {
        ...
        return wkof.Settings.load(script_id, defaults)
            .then((settings) => settings) // Type cast
    ...
    function settings_pre_open(dialog) {
        ...
        const buttons = (type) => `<div class="list_buttons">` +
            `<button type="button" ref="${type}" action="new" class="ui-button ui-corner-all ui-widget" title="Create a new ${type}">${Icons.customIconTxt('plus')}</button>` +
            `<button type="button" ref="${type}" action="up" class="ui-button ui-corner-all ui-widget" title="Move the selected ${type} up in the list">${Icons.customIconTxt('arrow-up')}</button>` +
            `<button type="button" ref="${type}" action="down" class="ui-button ui-corner-all ui-widget" title="Move the selected ${type} down in the list">${Icons.customIconTxt('arrow-down')}</button>` +
            `<button type="button" ref="${type}" action="delete" class="ui-button ui-corner-all ui-widget" title="Delete the selected ${type}">${Icons.customIconTxt('trash')}</button>` +
            `</div>`;
        ...
        const parent = dialog.find(`#${script_id}_burn_bell`).closest('div');
                ...
                try {
                    switch ($(e.target).attr('name')) {
                        case 'selected_preset':
                            const originalPreset = settings.presets[Number($(e.target).find(':selected').attr('name'))];
                        ...
                        case 'selected_action':
                            const originalAction = settings.presets[settings.selected_preset].actions[Number($(e.target).find(':selected').attr('name'))];
    ...
    function get_default_presets() {
        if (!!wkof.file_cache.dir[`wkof.settings.${script_id}`])
            return []; // If user already change settings don't include these
        const get_default_sort_by_level_action = () => $.extend(true, get_action_defaults(), {
            name: 'Sort by level',
            type: 'sort',
            sort: { type: 'level' },
        });
        const get_default_sort_by_type_action = () => $.extend(true, get_action_defaults(), {
        ...
        const learned = $.extend(true, get_preset_defaults(), {
            name: 'Learned',
            available_on: { reviews: false, lessons: false, extra_study: false, self_study: true },
            actions: [
                $.extend(true, get_action_defaults(), {
    ...
    function get_action_defaults() {
        ...
        for (let type of ['level', 'srs', 'leech', 'overdue', 'overdue_absolute']) {
    ...
    function populate_action_settings(config) {
        ...
        for (let [name, filter] of Object.entries(wkof.ItemData.registry.sources.wk_items.filters)) {
            if (filter.no_ui)
                continue;
            // Add to dropdown
            const filter_type = config.content.filter_type;
            filter_type.content[name] = (_a = filter.label) !== null && _a !== void 0 ? _a : 'Filter Value';
            // Add filter values
        ...
        const numerical_sort_config = (type) => ({
            type: 'dropdown',
            default: 'asc',
            label: 'Order',
    ...
    function refresh_action() {
        // Set action type
        const type = $(`#${script_id}_action_type`).val();
    ...
    function refresh_active_preset_selection() {
        ...
        function populate_preset_pickers(type) {
            const elem = $(`#reorder_omega_active_presets select[name="${type}"]`);
            ...
            // Insert into dialog
            const html = presets
                .map(([i, preset]) => `<option name="${i}" ${Number(i) == selected ? 'selected' : ''}>${preset.name}</option>`)
                .join('');
            elem.html(html);
        }
    }
    ...
    function list_button_pressed(e) {
        const ref = e.currentTarget.attributes.ref.value;
        const btn = e.currentTarget.attributes.action.value;
        const elem = $(`#${script_id}_active_` + ref);
        let default_item, root;
        if (ref === 'preset') {
        ...
        if (btn === 'new')
            $(`#${script_id}_${ref}_name`).focus().select();
    }
            ...
            switch (type) {
                case 'preset':
                    if (!obj.preset)
                        return;
                    // Add in defaults
                    obj.preset.actions = obj.preset.actions.map((action) => {
    ...
    function update_bell_audio() {
        const custom_base_64_bell = localStorage.getItem(`${script_id}_burn_bell_base64_audio`);
        const custom_bell = localStorage.getItem(`${script_id}_custom_burn_bell`);
        if (custom_base_64_bell)
            burn_bell_audio.src = `data:audio/mp3;base64,${custom_base_64_bell}`;
        else if (custom_bell)
